...
import { PurchaseDebitCreditNote } from "./DebitCreditNote/PurchaseDebitCreditNote";
import { PurchasePOList } from "./PurchasePO/PurchasePOList";
import ReactPaginate from "react-paginate";
import { RxDoubleArrowLeft, RxDoubleArrowRight } from "react-icons/rx";
import { SlArrowLeft, SlArrowRight } from "react-icons/sl";
import { MdKeyboardArrowDown, MdKeyboardArrowUp } from "react-icons/md";
import {
  fetchFilterPurchasPo,
  fetchPurchasePo,
  fetchSearchPurchasePo,
} from "../../Redux/Slice/PurchaseModule/PurchasePOThunk";
import { CreatePurchasePO } from "./PurchasePO/CreatePurchasePO";
import { ViewPurchasePO } from "./PurchasePO/ViewPurchasePO";
import {
  fetchFilterPurchasGrn,
  fetchPurchaseGRN,
  fetchSearchPurchaseGRN,
} from "../../Redux/Slice/PurchaseModule/PurchaseGRNThunk";
import { CreatePurchaseGrn } from "./GRN/CreatePurchaseGrn";
import dayjs from "dayjs";
import { ViewPurchaseGrn } from "./GRN/ViewPurchaseGrn";
import { CreateBillEntry } from "./BillEntry/CreateBIllEntry";
import { PurchaseBillEntryList } from "./BillEntry/PurchaseBillEntryList";
import {
  fetchBillEntry,
  fetchfilterBillEntry,
  fetchSearchedBillEntry,
} from "../../Redux/Slice/PurchaseModule/PurchaseBillEntryThunk";
...
import { CreatePurchaseReturn } from "./PurchaseReturn/CreatePurchaseReturn";
import {
  fetchReturnPO,
  filterPurchaseRtn,
  searchPurchaseRtn,
} from "../../Redux/Slice/PurchaseModule/ReturnPurchaseThunk";
import { ViewPurchaseReturn } from "./PurchaseReturn/ViewPurchaseReturn";
import { SubmitPurchasePOExcel } from "../../API/Purchase/PurchasePO";
import { SubmitPurchaseRtnExcel } from "../../API/Purchase/ReturnPurchase";

const { RangePicker } = DatePicker;

export default function Purchase() {
  const [tabName, setTabName] = useState("PurchaseOrder");
  ...
  const [purchasePOID, setPurchasePOID] = useState();
  const [purchasePOData, setPurchasePoData] = useState();
  const [PurchaseStatus, setPurchaseStatus] = useState("Pending");
  // ----------------------------------------Purchase GRN------------------------------------------
  const [purchaseGrnID, setPurchaseGrnID] = useState();
  const [purchaseGrnData, setPurchaseGrnData] = useState();
  // -------------------------------------------Purchase Bill Entry-----------------------------------
  ...
  const { returnPurchase, return_loading, return_error } = useSelector(
    (state) => state?.purchaseReturn
  );

  console.log("Pruchase Return :", return_error);

  const { purchase_PO, purchase_loading, purchase_error } = useSelector(
    (state) => state?.purchasingPo
  );
  ...
  const { billEntry, billEntry_Loading, billEntry_Error } = useSelector(
    (state) => state?.purchasingBillEntry
  );
  ...
  const closeModal = () => {
    ...
    setPurchaseGrnID(null);
    setPurchaseReturnID(null);
  };

  const handleSearchChange = (e) => {
    const value = e.target.value;
    if (tabName === "PurchaseOrder") {
      setSearchTerm(value);
      dispatch(fetchSearchPurchasePo(value)); // dispatch search
    } else {
      setSearchTerm(value);
      dispatch(searchPurchaseRtn(value)); // dispatch search
    }
  };
  ...
  const handleStatusChange = (value) => {
    let selectedStatus = "";
    if (tabName === "PurchaseOrder") {
      if (value === 1) selectedStatus = 1;
      else if (value === 2) selectedStatus = 2;
      else if (value === 3) selectedStatus = 3;
      setStatus(selectedStatus);
    } else if (tabName === "GRN") {
      if (value === 1) selectedStatus = 1;
      else if (value === 2) selectedStatus = 2;
    } else if (tabName === "BillEntry") {
      if (value === 1) selectedStatus = 1;
      else if (value === 2) selectedStatus = 2;
      else if (value === 3) selectedStatus = 3;
    }
  ...
  const applyFilters = (fromDate, toDate, status) => {
    ...
    switch (tabName) {
      case "PurchaseOrder":
        dispatch(fetchFilterPurchasPo(filters));
        break;
      default:
        dispatch(filterPurchaseRtn(filters));
    }
  };
  ...
  const toggleTab = (key) => {
    ...
    if (key === "PurchaseOrder") {
      dispatch(
        fetchFilterPurchasPo({ fromDate: null, toDate: null, status: "" })
      );
    } else if (key === "GRN") {
      dispatch(
        fetchFilterPurchasGrn({ fromDate: null, toDate: null, status: "" })
      );
    } else if (key === "BillEntry") {
      dispatch(
        fetchfilterBillEntry({ fromDate: null, toDate: null, status: "" })
      );
    }
  };
    ...
    const handleExcelSubmit = async (file) => {
      ...
      try {
        let response;
        if(tabName === 'PurchaseOrder'){
         response = await SubmitPurchasePOExcel(file);
        dispatch(fetchPurchasePo());
        } else{
        response = await SubmitPurchaseRtnExcel(file);
  ...
  const currentItems =
    tabName === "PurchaseOrder"
      ? purchase_PO?.slice(indexOfFirstItem, indexOfLastItem)
      : tabName === "GRN"
      ? purchaseGrn?.slice(indexOfFirstItem, indexOfLastItem)
      : tabName === "BillEntry"
      ? billEntry?.slice(indexOfFirstItem, indexOfLastItem)
      : returnPurchase?.slice(indexOfFirstItem, indexOfLastItem);

  const pageCount =
    tabName === "PurchaseOrder"
      ? Math.ceil(purchase_PO?.length / itemsPerPage)
      : tabName === "GRN"
      ? Math.ceil(purchaseGrn?.length / itemsPerPage)
      : tabName === "BillEntry"
      ? Math.ceil(billEntry?.length / itemsPerPage)
      : Math.ceil(returnPurchase?.length / itemsPerPage);
  ...
  const handleExport = () => {
    ...
    if (excelToExport?.length === 0) {
      alert("No data to export!");
      return;
    }

    const excelbook = XLSX.utils.book_new();
    const excelsheet = XLSX.utils.json_to_sheet(excelToExport);
    ...
    let fileName = "";
    if (tabName === "PurchaseOrder") {
      fileName = `purchaseOrder_${currentDate}.xlsx`;
    } else if (tabName === "GRN") {
      fileName = `GRN_${currentDate}.xlsx`;
    } else if (tabName === "BillEntry") {
      fileName = `billEntry_${currentDate}.xlsx`;
    } else {
  ...
  return (
    <>
          ...
          <div className="flex gap-x-[2vw] ml-[-1vw]">
            <div>
              <h1
                ...
                onClick={() => toggleTab("PurchaseOrder")}
              >
                Purchase Order
              </h1>
            </div>
            <div>
              <h1
                ...
                onClick={() => toggleTab("PurchaseReturn")}
              >
                Purchase Return
              </h1>
            </div>
              ...
              <div>
                ...
                >
                  ...
                  <span className="font-bold text-white text-[0.9vw]">
                    {tabName === "PurchaseOrder" ? "Add PO" : "Add Return"}
                  </span>
                </button>
              </div>
              <div>
            ...
            <div className="flex gap-[1.5vw] justify-end ">
              ...
              {tabName === "PurchaseOrder" && (
                <div className="">
                  ...
                  >
                    <Select
                      className="custom-select h-[2.2vw] relative flex justify-center items-center w-[12vw] rounded-[1vw] shadow-md placeholder:text-[1vw]"
                      allowClear
        ...
        {tabName === "PurchaseOrder" ? (
          <div>
            <PurchasePOList
              ...
              isView={isView}
              setIsView={setIsView}
              setPurchasePOID={setPurchasePOID}
          ...
          <div>
            <PurchaseReturnList
              ...
              setPurchaseReturnData={setPurchaseReturnData}
              setOpenAddModal={setOpenAddModal}
              isView={isView}
              setIsView={setIsView}
            />
        ...
        <div className="flex">
          {tabName === "PurchaseOrder" ? (
            <p className="text-[0.8vw]">
              ...
              {Math.min(itemOffset + itemsPerPage, purchase_PO?.length)} of{" "}
              {purchase_PO?.length} items
            </p>
          ) : tabName === "GRN" ? (
            <p className="text-[0.8vw]">
              Showing {itemOffset + 1} -{" "}
              {Math.min(itemOffset + itemsPerPage, purchaseGrn?.length)} of{" "}
              {purchaseGrn?.length} items
            </p>
          ) : tabName === "BillEntry" ? (
            <p className="text-[0.8vw]">
              Showing {itemOffset + 1} -{" "}
              {Math.min(itemOffset + itemsPerPage, billEntry?.length)} of{" "}
              {billEntry?.length} items
            </p>
          ) : (
            []
          )}
      ...
      <Modal
        ...
        onClose={closeModal}
      >
        {tabName === "PurchaseOrder" && generateModal === false ? (
          <CreatePurchasePO
