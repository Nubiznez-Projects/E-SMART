...
//const DCItems = invoiceData?.DCItems || [];
//const grossAmount = DCItems.reduce((sum, item) => sum + item.Amount, 0);

  //const taxValue = (grossAmount * TAX_RATE) / 100;
  //const totalAmount = grossAmount + taxValue;
  ...
  const parseItems = (raw) => {
    ...
    invoiceData?.map((item) => ({
      ...
      Items: parseItems(item?.DCItems?.length ? item.DCItems : poData?.DCItems).map(
        (item) => ({
          HSN: item.HSN || "",
  ...
  const columns = [
    {
      ...
      width: "15vw",
      render: (row) => {
        return <div className="text-[0.7vw]">{row?.TotalDCQty}</div>;
      },
    },
    {
   ...
   console.log(row, row?.Items, gross, "gross")
